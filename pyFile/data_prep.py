...
### if the input is an jpg image
# path = "pic.jpg"
# input_height = 28
# input_width = 28
# img = load_img(path, grayscale=True, target_size = (input_height, input_width))
# img = img_to_array(img, data_format='channels_first')
# np.savetxt('{0}/{1}_0.txt'.format(path_image, os.path.basename(os.path.normpath("pic.jpg")).split(".")[0]), img[0], fmt='%.8e')
...
np.savetxt('{0}/{1}_0.txt'.format(path_image, os.path.basename(os.path.normpath(path)).split(".")[0]), img[0], fmt='%.8e')