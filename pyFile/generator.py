...
def generate(model):
    ...
    f.write('\tfloat ***img;\n\timg = load_RGB("input", {0}, {1},{2});\n\n'.format(config[0]['batch_input_shape'][1],config[0]['batch_input_shape'][2], config[0]['batch_input_shape'][3]))
    f.write('\tTensor *t;\n\tt = make_tensor({0}, {1}, {2}, img);\n\n'.format(config[0]['batch_input_shape'][1],config[0]['batch_input_shape'][2], config[0]['batch_input_shape'][3]))
...
def main():
    ...
    generate(keras.models.load_model("my_model.h5", custom_objects={'my_mape': my_mape}))
    extract_weights(keras.models.load_model("my_model.h5", custom_objects={'my_mape': my_mape}))
